...
| Right-click menu | `ui.menu` (`lua/ui/menu/README.md`) — the general menu on `<RightMouse>` (plus an optional key at the cursor), drawn by `ui.contextmenu`: a fly-out per installed sister plugin (`<plugin>.integrations.menu`, each with a three-layer opt-out: installed, ui.nvim's `integrations.<name>`, the plugin's own switch), the general sections Code/Clipboard/File/Delete/Tools (destructive entries off by default), and rows of your own by plugin/filetype/section. "Copy/Delete Marked" act on the selection captured when the menu opened. Explicit-only: `ui.setup({ menu = { ... } })`; the first open is warmed up in idle time (`prewarm`) so lazy sister plugins do not stall it |
