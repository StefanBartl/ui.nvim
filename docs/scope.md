...
| Context | `ui.context` — a sticky code-context overlay: the enclosing function/class/loop lines that scrolled off the top, pinned over the window's first rows (Tree-sitter ancestor walk from the first visible line, node types matched by pattern rather than per-grammar queries, drawn in a non-focusable `relative="win"` float with the source line numbers in the gutter; in Markdown the pinned heading chain, capped by heading level and by row count). Off by default; `ui.setup({ sticky = true })` (or `context = true`) or `:UI sticky` |
