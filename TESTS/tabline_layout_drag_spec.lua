...
describe("ui.tabline.layout.slot_at", function()
  ...
  describe("chip_run_bounds", function()
    it("spans from the first chip's start to the last chip's end", function()
      layout.record("", { 10, 11, 12 }, { "aaaa", "bbbbbb", "cc" })
      local left, right = layout.chip_run_bounds()
      assert.equals(1, left)
      assert.equals(12, right) -- 4 + 6 + 2
    end)

    it("starts after the prefix", function()
      layout.record("    ", { 10 }, { "aaaa" })
      local left, right = layout.chip_run_bounds()
      assert.equals(5, left)
      assert.equals(8, right)
    end)

    it("is nil, nil before any render, or once every chip has been dropped", function()
      layout.record("", {}, {})
      local left, right = layout.chip_run_bounds()
      assert.is_nil(left)
      assert.is_nil(right)
...
describe("ui.tabline.drag", function()
  ...
  describe("auto-scroll while held at an edge", function()
    it("arms the scroll timer when the pointer rests at the left edge", function()
      drag.begin(a)
      assert.is_false(scroll.is_active())
      pointer_at(1) -- column 1 of a chip run spanning 1..30 (see before_each)
      mapped("<LeftDrag>")()
      assert.is_true(scroll.is_active())
    end)

    it("arms the scroll timer when the pointer rests at the right edge", function()
      drag.begin(a)
      pointer_at(29) -- within EDGE_COLS of the right edge (30)
      mapped("<LeftDrag>")()
      assert.is_true(scroll.is_active())
    end)

    it("does not arm the timer well inside the bar", function()
      drag.begin(a)
      pointer_at(15)
      mapped("<LeftDrag>")()
      assert.is_false(scroll.is_active())
    ...
    it("disarms once the pointer moves back off the edge", function()
      drag.begin(a)
      pointer_at(1)
      mapped("<LeftDrag>")()
      assert.is_true(scroll.is_active())
    ...
    it("cancel() (drag end) stops the timer and forgets the offset", function()
      drag.begin(a)
      pointer_at(1)
      mapped("<LeftDrag>")()
      assert.is_true(scroll.is_active())
